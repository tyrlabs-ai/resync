---
name: reposync
description: Set up and work in RepoSync-managed Git projects, including installing the CLI, authenticating with a provider, initializing, joining, or forking project identity, enrolling SSH peers, verifying synchronization, and following the required commit cadence. Use whenever the current repository contains .resync.toml, when starting or diagnosing RepoSync, or when moving work between machines or agents.
---

# RepoSync

Use RepoSync to propagate committed Git history between cooperating checkouts while preserving a stable hosted project identity. Treat a repository containing `.resync.toml` as RepoSync-managed.
...
5. Let enrollment create `.resync.toml`, install the project-local RepoSync skill plus Codex and Git lifecycle hooks, and start or reconnect the per-user daemon. Do not run `install-adapters` or `daemon` during normal setup; those are repair and diagnostic commands.
